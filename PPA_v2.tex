...
We also consider a slippage in costs due to inexperienced management. Let $\tau, y^{*}$  stand for, respectively, the impact of number of years of lack of experience on cost slippage and the optimal number of years of managerial experience; and $y$ represent the actual number of years of experience. In equation (\ref{eqInvestmentCost}) the percentage increase in capital with respect to financial strength ($h$) is represented by $\mu$. The financial strength is captured by %(\ref{eqFinanceStrenght})  
as the ratio between the bank warranties and the fixed costs of the project.  
...
\begin{subequations}\label{eqBuyersProblem}
	...
	&\qquad c = c_I(1-l)+c_Ll+\tau_c(y^{*}-y)^{+} \label{eqBuyerMarginalCost}\\
	&\qquad o = o_I(1-l)+o_Ll+\tau_o(y^{*}-y)^{+} \label{eqBuyerOperationalCost}\\
	&\qquad f = f_I(1-l)+f_Ll+\tau_f(y^{*}-y)^{+}+\mu h \label{eqBuyerInvestmentCost}\\
	...
	&\qquad \theta = \theta_0-\epsilon_l l -\epsilon_y \left(y^{*}-y\right)^{+}+\epsilon_h h \label{eqBuyerTheta}
	\end{align}
\end{subequations}
