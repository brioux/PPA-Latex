...
The profit function considered (\ref{eqProfit}) depends on the specific technology being auctioned, and on possible penalties associated to the call for projects. $P$ is the average energy price; $\delta$ stands for the capacity price; $c$  is the marginal cost; $o$  fixed operational costs; $f$  is the fixed investment cost; $l$  is the percentage of local content, $Q$ is production and $K$ is capacity. $y$ is the number of years of experience of the management team, and $h$ is an index of the financial strength of the project.


\begin{equation} \label{eqProfit}
\pi(\delta,l,h,y) = (P-c)Q+(\delta-o-f)K
\end{equation}


As local content is a requirement that some governments find necessary to develop industries that are not yet competitive at international prices, it leads to higher industry costs (due to lower productivity from local equipment and local workers). For this reason, we can approximate the relationship between local content and the cost structure of the firms as represented in (\ref{eqMarginalCost}) to (\ref{eqInvestmentCost}), in which $c_I, o_I, f_I$ , stand for the expected marginal, operational and investment international costs, and $c_L, o_L, f_L$  represent the equivalent costs when the plants are built using local content only. 
We also consider a slippage in costs due to inexperienced management. Let $\tau, y^{*}$  stand for, respectively, the impact of number of years of lack of experience on cost slippage and the optimal number of years of managerial experience; and $y$ represent the actual number of years of experience. In equation (\ref{eqInvestmentCost}) the percentage increase in capital with respect to financial strength ($h$) is represented by $\mu$. The financial strength is described by (\ref{eqFinanceStrenght}) as the ratio between the bank warranties ($W$) and the fixed costs of the project.  We also include the percentage increase in capital cost based on the years of experience of the management team as the parameter $\xi$.


\begin{equation} \label{eqMarginalCost}
c = c_I(1-l)+c_Ll+\tau_c(y^{*}-y)^{+}
\end{equation}

\begin{equation} \label{eqOperationalCost}
o = o_I(1-l)+o_Ll+\tau_o(y^{*}-y)^{+}
\end{equation}

\begin{equation} \label{eqInvestmentCost}
f = \left(f_I(1-l)+f_Ll+\tau_f(y^{*}-y)^{+}\right)\left(1+\mu h+y \xi\right)
\end{equation}

\begin{equation} \label{eqFinanceStrenght}
h = \frac{W}{(o+f)K}
\end{equation}


The energy supply is represented by (\ref{eqEnergySupply}) and it is a function of the available capacity. The targeted proportion of capacity available for production, $\theta$, is represented by (\ref{eqTheta}), i.e., the process governing production, which is a function of the local content: the larger the proportion of local content the lower the proportion of capacity available and the faster the deterioration of the plant, $\epsilon_l$. It is also a function of the number of years of experience of the team below to optimal level, $\epsilon_y$, and of the financial strength of the project, $\epsilon_h$. Another important factor that influences the project feasibility is the risk in project execution, including the risks associated with the project, the generator, the buyer, and the social economic environment. Among the several factors conditioning these risks we would emphasize the degree of subcontracting, the degree of technological challenges, safety hazards, knowledge of the management team, financial strength of the firm, regulatory changes, availability of qualified staff, fuel price fluctuations. The project feasibility needs, therefore, to take into account the risk of execution, which depends on the number of years of experience of the management team $(y)$, on the proportion of local content $(l)$. We would hypothesize that local content leads to higher risk of increased costs whereas years of experience decreases the risk. 
...
\theta = \theta_0-\epsilon_l l -\epsilon_y \left(y^{*}-y\right)^{+}+\epsilon_h h
\end{equation}
...
Finally, the buyer can also publish a demand for capacity in the call for projects. The advantage of such a function is that it provides incentives for the generators to reduce the capacity payment in order to develop larger projects. The capacity demand function is represented by (\ref{eqCapacityDemand}): $k_0$ is the minimum size of the project, which is paid the maximum capacity price, $\bar{\delta}$ , and $g$ is the increase in the size of the production capacity per monetary unit of decrease in the capacity price.


\begin{equation} \label{eqCapacityDemand}
K = k_0 +g\left(\bar{\delta}-\delta\right)
\end{equation}
...
\begin{subequations}\label{eqBuyersProblem}
	...
	\quad \left(P-c\right)Q+\left(\delta-o-f\right)K \label{eqBuyerObjective}\\
	&\quad \mbox{{\it subject to}:}\notag\\
	&\qquad	 \delta_s \leq \delta_{s}^{0}\label{eqBuyerCapacityPrice} \qquad \quad \left[\omega_{\delta}\right] \\
	&\qquad	 l_s \geq l_{s}^{0}\label{eqBuyerLocal} \qquad \quad \left[\omega_{l}\right]\\
	&\qquad	 h_s \geq h_{s}^{0}\label{eqBuyerFinance} \qquad \quad \left[\omega_{h}\right]\\
	&\qquad	 y_s \geq y_{s}^{0}\label{eqBuyerExperience} \qquad \quad \left[\omega_{y}\right]\\
	&\qquad	 \theta_s \leq \theta_{s}^{0}\label{eqBuyerEfficiency} \qquad \quad \left[\omega_{\theta}\right]\\
	&\qquad	 P_s \leq P_{s}^{0}\label{eqBuyerEnergyPrice} \qquad \quad \left[\omega_{P}\right]\\
	&\qquad	 Q_s \geq Q_{s}^{0}\label{eqBuyerEnergyProduction} \qquad \quad \left[\omega_{Q}\right]\\
	&\qquad	 K_s \geq K_{s}^{0}\label{eqBuyerCapacity} \qquad \quad \left[\omega_{K}\right]\\
...
(\ref{eqBuyerEfficiency}) represents the minimum efficiency requirements. (\ref{eqBuyerEnergyPrice}) is the target for the standardized energy price. (\ref{eqBuyerEnergyProduction}) specifies the target for the expected production and (\ref{eqBuyerCapacity}) for the project size. 
