...
\begin{subequations}\label{eqBuyersProblem}
	...
	\quad %\phi(\delta_s, h_s, l_s, p_s, y_s) = 
	 \omega_{\delta}(\delta_{s}^{0}-\delta_s)+\omega_{h}(h_s-h_{s}^{0})+\omega_{l}(l_{s} - l_{s}^{0})+\omega_{p}(p_{s}^{0}-p_s)+\omega_{y}(y_s-y_{s}^{0}) \label{eqBuyerObjective}\\
	&\quad \mbox{{\it subject to}:}\notag\\
%	&\qquad	 \delta_s \leq \delta_{s}^{0}\label{eqBuyerCapacityPrice}  \\
%	&\qquad	 h_s \geq h_{s}^{0}\label{eqBuyerFinance} \\
%	&\qquad	 l_s \geq l_{s}^{0}\label{eqBuyerLocal}\\	
%	&\qquad	 p_s \leq p_{s}^{0}\label{eqBuyerEnergyPrice}\\
%	&\qquad	 y_s \geq y_{s}^{0}\label{eqBuyerExperience} \\
	&\qquad	 \omega_{\delta}+\omega_{h}+\omega_{l}+\omega_{p}+\omega_{y}=1\label{eqBuyerNormal} \\		
	&\qquad \mathop {Maximize}\limits_{\delta_s, h_s, l_s, p_s, y_s}
	\quad \rho(\delta_s, h_s, l_s,p_s,y_s)\left(\left(p-c\right)q+\left(\delta-f\right)k\right) \label{eqGeneratorObjective}\\ 
	&\qquad \quad \mbox{{\it subject to}:}\notag\\
	&\qquad	\qquad \delta = \mathop{\mathbb{E}}\left(\delta\right)+\sigma\left(\delta\right)\delta_s\label{eqDeltaStd}\\
	&\qquad	\qquad h = \mathop{\mathbb{E}}\left(h\right)+\sigma\left(h\right)h_s\label{eqHStd}\\
	&\qquad	\qquad l = \mathop{\mathbb{E}}\left(l\right)+\sigma\left(l\right)l_s\label{eqLStd}\\
	&\qquad	\qquad p = \mathop{\mathbb{E}}\left(p\right)+\sigma\left(p\right)p_s\label{eqPStd}\\
	&\qquad	\qquad y = \mathop{\mathbb{E}}\left(y\right)+\sigma\left(y\right)y_s\label{eqYStd}\\
	...
	&\qquad	\qquad \rho(\delta_s, h_s, l_s, p_s,y_s)=\frac{1}{1+e^{-\phi(\delta_s, h_s, l_s, p_s, y_s)}} \label{eqGeneratorProbability}\\
